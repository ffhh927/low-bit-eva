...
from kfac.utils import get_vector_a, get_vector_g, batch_transfer_to_cpu_inplace
import logging
...
class KFAC(optim.Optimizer):
    ...
    def __init__(self,
        ...
        self.cpu_executor_P = ThreadPoolExecutor(max_workers=1)
        self.tasks = []
    ...
    def batch_transfer_to_cpu_inplace(self, tensor_list):
        """
    ...
    def batch_transfer_to_gpu_inplace(self, tensor_list, device):
        """
    ...
    def _cpu_quantization_taskA(self, module, new_fp32):
        ...
        self.m_a[module][1], self.quant_state_a[module] = B_F.quantize_blockwise(new_fp32, code=self.code, blocksize=self.block_size)
        # self.quant_state_a[module].absmax = self.quant_state_a[module].absmax.to(self.device, non_blocking=True)
        # self.quant_state_a[module].code = self.quant_state_a[module].code.to(self.device)
    ...
    def _cpu_quantization_taskG(self, module, new_fp32):
        ...
        self.m_g[module][1], self.quant_state_g[module] = B_F.quantize_blockwise(new_fp32, code=self.code, blocksize=self.block_size)
        ...
        # self.quant_state_g[module].code = self.quant_state_g[module].code.to(self.device)
    ...
    #         ma = torch.cat((B_F.dequantize_blockwise(A=self.m_a[module][1], absmax=self.quant_state_a[module].absmax, blocksize=self.block_size, code=self.code), self.m_a[module][0].unsqueeze(0)), dim=0).view(-1, 1)
    #         mg = torch.cat((self.m_g[module][0].unsqueeze(0), B_F.dequantize_blockwise(A=self.m_g[module][1], absmax=self.quant_state_g[module].absmax, blocksize=self.block_size, code=self.code)), dim=0).view(-1, 1)
    ...
    def _cpu_quantization_taskP(self, module):
            # 解量化 -> 合并 -> 重新量化
        # self.batch_transfer_to_cpu_inplace([self.quant_state_a[module].absmax, self.m_a[module][1], self.quant_state_g[module].absmax, self.m_g[module][1]])
        # self.quant_state_g[module].absmax = restored_tensors
        # self.quant_state_g[module].absmax = self.quant_state_g[module].absmax.cpu(non_blocking=True)
        # new_fp32 = new_fp32.cpu(non_blocking=True)
        # m_g_old = B_F.dequantize_blockwise(self.m_g[module][1].cpu(), self.quant_state_g[module], blocksize=self.block_size)
        # m_a = B_F.dequantize_blockwise(self.m_a[module][1], self.quant_state_a[module], blocksize=self.block_size).to(self.device).view(-1, 1)
        # m_g = B_F.dequantize_blockwise(self.m_g[module][1], self.quant_state_g[module], blocksize=self.block_size).to(self.device).view(-1, 1)
        self.ma_buf = torch.cat((B_F.dequantize_blockwise(A=self.m_a[module][1], absmax=self.quant_state_a[module].absmax, blocksize=self.block_size, code=self.code).to(self.device), self.m_a[module][0].unsqueeze(0)), dim=0).view(-1, 1)
        self.mg_buf = torch.cat((self.m_g[module][0].unsqueeze(0), B_F.dequantize_blockwise(A=self.m_g[module][1], absmax=self.quant_state_g[module].absmax, blocksize=self.block_size, code=self.code).to(self.device)), dim=0).view(-1, 1)
        # print(self.ma.shape)
        # print(self.mg.shape)
        # return ma, mg
    ...
    def _precondition_grads(self):
        ...
        wait(self.tasks)
        self.tasks.clear()
        self.ma = torch.cat((B_F.dequantize_blockwise(A=self.m_a[self.modules[0]][1], absmax=self.quant_state_a[self.modules[0]].absmax, blocksize=self.block_size, code=self.code), self.m_a[self.modules[0]][0].unsqueeze(0)), dim=0).view(-1, 1)
        self.mg = torch.cat((self.m_g[self.modules[0]][0].unsqueeze(0), B_F.dequantize_blockwise(A=self.m_g[self.modules[0]][1], absmax=self.quant_state_g[self.modules[0]].absmax, blocksize=self.block_size, code=self.code)), dim=0).view(-1, 1)

        # print("test")
        for i, module in enumerate(self.modules):
            # print("test")
            # print(i)
            # print(self.m_a[module][1].shape)
            # print("test")
            if i + 1 < len(self.modules):
                future = self.cpu_executor_P.submit(
                            self._cpu_quantization_taskP, 
                            self.modules[i + 1]
                        )
            # ma = torch.cat((B_F.dequantize_blockwise(A=self.m_a[module][1], absmax=self.quant_state_a[module].absmax, blocksize=self.block_size, code=self.code), self.m_a[module][0].unsqueeze(0)), dim=0).view(-1, 1)
            # mg = torch.cat((self.m_g[module][0].unsqueeze(0), B_F.dequantize_blockwise(A=self.m_g[module][1], absmax=self.quant_state_g[module].absmax, blocksize=self.block_size, code=self.code)), dim=0).view(-1, 1)
            ...
            # compute intermediate states
            a = (self.ma.T @ self.ma).item()
            g = (self.mg.T @ self.mg).item()
            ag = (self.mg.T @ grad @ self.ma).item()

            # compute preconditioned grads
            v = (self.mg @ self.ma.T).mul_(-ag/(a * g + self.damping))           
            v.add_(grad)
            v.div_(self.damping)
