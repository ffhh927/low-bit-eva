...
class KFAC(optim.Optimizer):
    ...
    def _cpu_quantization_taskA(self, module, new_fp32):
        ...
        # print("test")
        # print(m_a_old.shape)
        m_a_old =  torch.cat((m_a_old, self.m_a[module][0].cpu().unsqueeze(0)))
        # print("test1")
        # m_a_old = B_F.dequantize_blockwise(self.m_a[module][1], self.quant_state_a[module], blocksize=self.block_size)       
        # print(f"[{threading.current_thread().name}] a m_a_old: {m_a_old}")
        # print(m_a_old.shape)
        # print(new_fp32.shape)
        ...
        self.m_a[module]=[new_fp32[-1].to(self.device, non_blocking=True), new_fp8.to(self.device, non_blocking=True)] 
        # print("test2")
