...
class KFAC(optim.Optimizer):
        ...
        if self.hook_enabled and torch.is_grad_enabled() and self.steps % self.fac_update_freq == 0:
            with torch.no_grad():
                ...
                if module not in self.m_a:
                    ...
                    # print("start")
                    self.buf_a[module] = new_fp32[:-1]
                    
                    if len(self.buf_a) > len(self.modules)/3 or module == self.modules[-1]:
                        self.tasks.append(self.cpu_executor.submit(
                            self._cpu_quantization_taskA, self.buf_a.copy()
                        ))
                        self.buf_a.clear()
                    # self.tasks.append(self.cpu_executor.submit(
        ...
        if self.hook_enabled and self.steps % self.fac_update_freq == 0:
            with torch.no_grad():
                ...
                if module not in self.m_g:
                    ...
                    #     print(param)
                    # print(self.modules.index(module))
                    # print(len(self.modules) * 2 / 3)
                    if self.modules.index(module) > int(len(self.modules) * 2 / 3):
                        self.buf_g[module] = new_fp32[1:]
                        # print("test")
                    elif self.modules.index(module) == int(len(self.modules) * 2 / 3):
                        self.buf_g[module] = new_fp32[1:]
                        self.tasks.append(self.cpu_executor.submit(
                            self._cpu_quantization_taskG, self.buf_g.copy()
                        ))
                        self.buf_g.clear()
                        # print("test1")
                    else:
    ...
    def _cpu_quantization_taskA(self, buf):
        # 解量化 -> 合并 -> 重新量化  
        # batch_transfer_to_cpu_inplace([self.quant_state_a[module].absmax, new_fp32, self.m_a[module][1]])
        # start_time = time.time()
       
        for module, new_fp32 in buf.items():
            self.quant_state_a[module].absmax = self.quant_state_a[module].absmax.cpu() 
            new_fp32 = new_fp32.cpu()
            self.m_a[module][1] = self.m_a[module][1].cpu()
            m_a_old = B_F.dequantize_blockwise(self.m_a[module][1], self.quant_state_a[module], blocksize=self.block_size)  
            # m_a_old = B_F.dequantize_blockwise(self.m_a[module][1], self.quant_state_a[module], blocksize=self.block_size)       
            # print(f"[{threading.current_thread().name}] a m_a_old: {m_a_old}")
            torch.lerp(m_a_old, new_fp32, self.factor_decay, out=new_fp32)
            # print(f"[{threading.current_thread().name}] a new_fp32: {new_fp32}")
        ...
        # print(f"[{threading.current_thread().name}] a new_fp8: {new_fp8}")
        # logging.info(f"Logging g :\n{mg}")
            
    def _cpu_quantization_taskG(self, buf):
            # 解量化 -> 合并 -> 重新量化
        ...
        for module, new_fp32 in buf.items():
            ...
            new_fp32 = new_fp32.cpu()
            self.m_g[module][1] = self.m_g[module][1].cpu()
    ...
    def _precondition_grads(self):
        ...
        for module in self.modules:
            ...
            # self.quant_state_a[module].absmax = self.quant_state_a[module].absmax.to(self.device)
